...
def ipa_pinyin(ipa):
    ...
    if ipa.startswith('p') and not(ipa.startswith('pʰ')): consonant = 'b'
        ...
        'on',
        'iȵ̍', 'uəȵ̍', 'yȵ̍',
        'iaŋ', 'uaŋ', 'oŋ', 'ioŋ', 'əŋ', 'iəŋ'
    ]: consonant = ''
    else: error = 1

    # 韵母
    if ipa.endswith('ɿ') or ipa.endswith('ɿ˗'):      vowel = 'ii'
    ...
    elif ipa.endswith('ɘ')    and not(ipa.endswith('uɘ')): vowel = 'e'
    elif ipa.endswith('ie'):   vowel = 'ie'
    elif ipa.endswith('uɘ'):   vowel = 'ue'    
    ...
    if syllable == 'ien':
        syllable = 'in'
    if error == 1:
        print("ERROR: {} don't has a difined pinyin".format(ipa))
        # quit()
    else:
        return syllable
